    ...
    def checkInclusion(self, s1, s2):
        ...
        while(end<len(s2)):
            if letter_count_s1[s2[end]] > 0: # if current character is present is s1
                ...
                if letter_count[s2[end]] == 0:
                    count-=1
            if end-start+1 < len_s1:
                end+=1
            elif end-start+1 == len_s1:
                if count == 0: return True   #  #if Counter of the actual matches with Counter of substring in current window -> the current substring is a permutation of s1
                if letter_count_s1[s2[start]] > 0:
                    letter_count[s2[start]]+=1
                    if letter_count[s2[start]] == 1: 
                        count+=1  #  #before moving the window check if the start index(which will be out of window as window moves) contributes to letter count. If yes then decrease the letter count of that letter by 1
                start+=1
                end+=1
            # print(letter_count, )
        return False
